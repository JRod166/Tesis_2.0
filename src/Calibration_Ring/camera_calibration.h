...
double CameraCalibration::calibrate_camera() {
	...
	//x => array[n] initial estimate
	//fvec =>  output of functions evaluated
	//tol =>  termination occurs when the algorithm estimates either that the relative error in the sum of squares is at most tol or that the relative error between x and the solution is at most tol.
	//info
	...
	*/
	//iwa => integer work array of length n
